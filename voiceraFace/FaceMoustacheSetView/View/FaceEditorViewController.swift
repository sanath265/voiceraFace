...
class FaceEditorViewController: UIViewController {
    ...
    @IBOutlet weak private var collectionView: UICollectionView! {
        didSet {
            let nib = UINib(nibName: "FaceEditorCollectionViewCell",
                            bundle: nil)
            collectionView.register(nib,
                                    forCellWithReuseIdentifier: "FaceEditorCollectionViewCell")
...
extension FaceEditorViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        if let faceNode = sceneView.scene.rootNode.childNode(withName: "mustache",
                                                             recursively: true) as? FaceNode {
...
extension FaceEditorViewController: AVCaptureFileOutputRecordingDelegate {
    ...
    @objc private func startRecording(_ sender: UITapGestureRecognizer) {
        if isRecording {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self.thumbnail = self.recordAR?.photo()
    ...
    private func stopRecording() {
        recordAR?.stopAndExport { [weak self] videoPath,
            permissionStatus,
            exported in
            guard let self else { return }
            ...
            if exported {
                DispatchQueue.main.async { [weak self] in
                    guard let self else { return }
                    self.showTagPopup(for: videoPath,
                                      thumbnail: (thumbnail ?? UIImage(systemName: "video")!))
                }
            } else {
                debugPrint("Failed to export video")
            }
        }
    ...
    private func generateThumbnail(for videoPath: URL, completion: @escaping (UIImage) -> Void) {
        ...
        DispatchQueue.global().async {
            if let cgImage = try? imageGenerator.copyCGImage(at: CMTime(seconds: 1
    ...
    private func showTagPopup(for videoPath: URL, thumbnail: UIImage) {
        let alertController = UIAlertController(title: "Enter Tag",
                                                message: "Add a tag for this recording.",
        ...
        let saveAction = UIAlertAction(title: "Save", style: .default) { [weak self] _ in
            ...
            let newRecording = NSEntityDescription.insertNewObject(forEntityName: "Recordings",
                                                                   into: context)
            
            // Save videoPath as String
            newRecording.setValue(videoPath.absoluteString,
                                  forKey: "videoPath")
            
            // Save thumbnail as Data
            let imageData = thumbnail.pngData()
            newRecording.setValue(imageData,
                                  forKey: "thumbnail")
            
            // Save duration and tag as String
            newRecording.setValue(duration,
                                  forKey: "duration")
            newRecording.setValue(tag,
                                  forKey: "tag")
            do {
                try context.save()
                debugPrint("Recording saved to Core Data")
            } catch {
                debugPrint("Error saving recording: \(error.localizedDescription)")
            }
            debugPrint("Recording saved with tag: \(tag)")
        }
    ...
    private func startTimer() {
        recordingTimer = Timer.scheduledTimer(timeInterval: 1.0,
                                              target: self, selector: #selector(updateRecordingTime),
                                              userInfo: nil,
                                              repeats: true)
    }
    ...
    @objc private func toggleCamera(_ sender: UITapGestureRecognizer) {
        let vc = RecordingGridViewController()
    ...
    func fileOutput(_ output: AVCaptureFileOutput,
        ...
        stopTimer()
        if let error = error {
            debugPrint("Error recording: \(error.localizedDescription)")
        } else {
    ...
    @objc func videoSavedToLibrary(_ videoPath: String?,
                                   didFinishSavingWithError error: Error?,
                                   contextInfo: UnsafeRawPointer) {
        if let error = error {
            debugPrint("Error saving video: \(error.localizedDescription)")
        } else {
            debugPrint("Video saved successfully!")
        }
    }
}
...
extension FaceEditorViewController: ARSCNViewDelegate {
    ...
    func updateFeatures(for node: SCNNode,
                        using anchor: ARFaceAnchor) {
        let mustacheIndices = [[2]]
